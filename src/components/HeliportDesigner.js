...
export default function HeliportDesigner({ mode = SIMULATOR_MODE.LATIHAN }) {
  ...
  async function submitToServer(mhs) {
    ...
    if (!res.ok) {
      ...
      throw new Error(err.error || "Gagal mengirim tugas ke server.");
    }
    return res.json();
  }
      ...
      <main className="mx-auto max-w-[1400px] px-4 pb-16">
          {isTugasMode && (
            <p className="mb-4 rounded-lg bg-amber-50 px-4 py-3 text-xs text-amber-900 ring-1 ring-amber-200">
              ...
              Tentukan nilai huruf A–O berdasarkan ketentuan KP 215, lalu desain layout dan tekan{" "}
              <b>Submit Tugas</b>. Hasil akan dikirim ke dosen dan PDF diunduh otomatis.
            </p>
          )}
          {isTugasMode && submitStatus === "success" && (
            <p className="mb-4 rounded-lg bg-emerald-50 px-4 py-3 text-xs text-emerald-800 ring-1 ring-emerald-200">
              Tugas berhasil dikirim ke dosen. PDF laporan telah diunduh.
            </p>
          )}
          {isTugasMode && submitStatus === "error" && (
            <p className="mb-4 rounded-lg bg-red-50 px-4 py-3 text-xs text-red-800 ring-1 ring-red-200">
              Gagal mengirim tugas. Periksa koneksi lalu coba lagi.
            </p>
          )}
